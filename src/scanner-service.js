...
class ScannerService {
  ...
  loadConfig() {
    return {
      ...
      accountConfig: process.env.ACCOUNT_CONFIG
        ? JSON.parse(process.env.ACCOUNT_CONFIG)
        : {},
  ...
  async performScan(url, options) {
    ...
    try {
      ...
      if (this.config.sessionToken) {
        scanResult.enhancedAnalysis =
          await this.getEnhancedAnalysis(scanResult);
      }
      ...
      console.log(
        `✅ Scan completed: ${url} - Score: ${scanResult.enhancedAnalysis?.enhancedScore || score}`,
      );
      return scanResult;
    } catch (error) {
      console.error(`❌ Scan failed for ${url}:`, error);
  ...
  calculateScore(results) {
    ...
    const score = Math.round(
      ((maxPossibleWeight - weightedViolations) / maxPossibleWeight) * 100,
    );
      ...
      const enhancedData = {
        ...
        forms: await page.evaluate(() => {
          return Array.from(document.querySelectorAll("form")).map((form) => ({
            ...
            labeledFields: form.querySelectorAll(
        ...
        visual: await page.evaluate(() => {
          return {
            colorScheme:
              ...
              "normal",
            hasHighContrast: !!window.matchMedia("(prefers-contrast: high)")
              .matches,
            hasReducedMotion: !!window.matchMedia(
        ...
        performance: await page.evaluate(() => {
          ...
          return {
            loadTime: perf
              ? Math.round(perf.loadEventEnd - perf.loadEventStart)
              : 0,
            domInteractive: perf
              ? Math.round(perf.domInteractive - perf.navigationStart)
              : 0,
  ...
  async getEnhancedAnalysis(scanResult) {
    if (!this.config.sessionToken || !this.config.apiUrl) {
      console.log(
        "⚠️  Skipping enhanced analysis - no session token or API URL",
      );
      return null;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.apiUrl}/api/v1/analysis/enhance`,
        {
          method: "POST",
          headers: {
